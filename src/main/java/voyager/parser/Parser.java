...
public final class Parser {
     ...
     *
     * @throws VoyagerException if the command or any of its arguments is invalid
     */
    public ParsedCommand parse(String rawInput) throws VoyagerException {
        String input = rawInput == null ? "" : rawInput.trim();
        if (input.isEmpty()) {
            throw new VoyagerException(INVALID_INPUT);
        }
        ...
        return switch (keyword) {
            ...
            default -> throw new VoyagerException(INVALID_INPUT);
        };
    }

    /** Validates and creates an activity command. */
    private static ParsedCommand parseActivity(String details) throws VoyagerException {
        if (details.isEmpty()) {
            throw new VoyagerException(INVALID_ACTIVITY_FORMAT);
        }
        ...
        try {
            ...
            throw new VoyagerException(INVALID_ACTIVITY_DATE);
        }
    }

    /** Validates stay syntax and chronological ISO dates. */
    private static ParsedCommand parseStay(String details) throws VoyagerException {
    ...
    private static ParsedCommand parseTransport(String details) throws VoyagerException {
    ...
    private static String[] parseThreeParts(String details, String errorMessage) throws VoyagerException {
        Matcher matcher = MARKER_PATTERN.matcher(details);
        if (!matcher.find() || !matcher.group(1).equalsIgnoreCase("from")) {
            throw new VoyagerException(errorMessage);
        }
    ...
    private static LocalDate[] validateStayDates(String fromText, String toText) throws VoyagerException {
        try {
            LocalDate fromDate = LocalDate.parse(fromText);
            LocalDate toDate = LocalDate.parse(toText);
            if (!fromDate.isBefore(toDate)) {
                throw new VoyagerException(INVALID_STAY_DATES);
            }
    ...
    private static ParsedCommand parseItemCommand(CommandType type, String details) throws VoyagerException {
        if (!details.matches("\\d+")) {
            throw new VoyagerException(INVALID_ITEM_NUMBER);
        }
        try {
    ...
    private static ParsedCommand parseArgumentlessCommand(CommandType type, String details) throws VoyagerException {
        if (!details.isEmpty()) {
            throw new VoyagerException(INVALID_INPUT);
        }
    ...
    private static ParsedCommand parseView(String details) throws VoyagerException {
        String[] fields = details.split("\\s+");
        if (details.isEmpty() || fields.length > 2) {
            throw new VoyagerException(INVALID_VIEW_DATE);
        }
        try {
    ...
    private static ParsedCommand parseFind(String details) throws VoyagerException {
        if (details.isEmpty()) {
            throw new VoyagerException(INVALID_FIND_FORMAT);
        }
